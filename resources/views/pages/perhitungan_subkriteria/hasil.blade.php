                        ...
                        @foreach ($kriteria->subkriterias as $outerIndex => $subkriteria2)
                            <tr class="text-center">
                                ...
                                @foreach ($kriteria->subkriterias as $innerIndex => $subkriteria3)
                                    <td class="border">
                                        {{-- validate if kriteria already inputed  --}}
                                        @if ((count($kriteria->subkriterias) * count($kriteria->subkriterias)) != count($perhitungans_all))
                                        <select name="{{$subkriteria2->nama_subkriteria . '[]'}}" class="form-select px-4 matrix_select" style="padding: 5px 20px" {{$innerIndex == $outerIndex ? 'disabled' : ''}} data-id="{{$innerIndex . ',' . $outerIndex}}">
                                        ...
                                        <select name="{{$subkriteria2->nama_subkriteria . '[]'}}" class="form-select px-4 matrix_select" style="padding: 5px 20px" {{$innerIndex == $outerIndex ? 'disabled' : ''}} data-id="{{$innerIndex . ',' . $outerIndex}}">
                                            @if (count($subkriteria2->perhitungansubkriterias))
                                                ...
                                                <?php 
                                                    $total_nilai_prioritas[$innerIndex] = ISSET($total_nilai_prioritas[$innerIndex]) ? ($total_nilai_prioritas[$innerIndex] + $subkriteria2->perhitungansubkriterias[$innerIndex]->nilai) : $subkriteria2->perhitungansubkriterias[$innerIndex]->nilai;
                                                ?>
                                                    <option value="{{$subkriteria2->perhitungansubkriterias[$innerIndex]->nilai}}" data-dynamic="true">{{number_format($subkriteria2->perhitungansubkriterias[$innerIndex]->nilai, 2)}}</option>
                                                @endif
                                            @endif
                                            @foreach (range(1,9) as $point)
                                                <option value="{{$point}}" >{{$point}}</option>
                ...
                <table class="table border">
                    ...
                    @foreach ($kriteria->subkriterias as $outerIndex2 => $subkriteria4)
                        <tr class="text-center">
                            ...
                            <td class="border">{{number_format($jml_nilai/count($kriteria->subkriterias), 2)}}</td>
                        </tr>
                        <?php $max_lamda += ($jml_nilai/count($kriteria->subkriterias)) * $total_nilai_prioritas[$outerIndex2]; ?>
                    @endforeach
                    </tbody>
                </table>
                <div class="col-12 my-2 w-70 mx-auto">
                    ...
                    <div class="row">
                        <div class="col-6">
                            ...
                            <p class="p-0 m-0" style="color: #333">CI = ({{number_format($max_lamda, 2)}} - {{$length}}) / {{$length-1}}</p>
                            <p class="p-0 m-0" style="color: #333">CI = {{number_format($ci, 2)}}</p>
                        </div>
                        <div class="col-6">
                            <p class="p-0 m-0" style="color: #333">Consistensi Ratio = CI/IR</p>
                            <p class="p-0 m-0" style="color: #333">CR = {{number_format(number_format($ci,2))}} / {{$nilai_index_random}}</p>
